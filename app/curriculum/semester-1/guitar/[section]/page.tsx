...
const FILE = '04-curriculum/semester-1/guitar-track.md'
...
async function getSectionContent(slug: string): Promise<string> {
  switch (slug) {
    case 'foundations':
      return getMarkdownSection(FILE, null, ['Month 1 Guitar Plan'])
    case 'month-1':
      return getMarkdownSection(FILE, 'Month 1 Guitar Plan', ['Month 2 Guitar Plan'])
    case 'month-2':
      return getMarkdownSection(FILE, 'Month 2 Guitar Plan', ['Month 3 Guitar Plan'])
    case 'month-3':
      return getMarkdownSection(FILE, 'Month 3 Guitar Plan', [])
    default:
      return ''
  }
}

export default async function GuitarSectionPage({
  params,
}: {
    ...
    <div style={{ backgroundColor: '#f8f7f4', minHeight: '100vh' }}>
      ...
      <div className="py-10 px-4 sm:px-6" style={{ backgroundColor: '#0f172a' }}>
        <div className="max-w-3xl mx-auto">
          <p className="text-slate-400 text-sm mb-3">
            ...
            {' / '}
            <Link href="/curriculum/semester-1/guitar" className="hover:text-amber-400 transition-colors">Guitar Track</Link>
            {' / '}
      ...
      <div className="border-t" style={{ borderColor: '#e2e8f0', backgroundColor: '#fff' }}>
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 flex items-center justify-between gap-4">
          {prevSection ? (
            <Link
              href={`/curriculum/semester-1/guitar/${prevSection.slug}`}
            ...
            <Link
              href="/curriculum/semester-1/guitar"
              className="flex-1 group flex items-center gap-3 p-4 rounded-xl border hover:border-slate-300 transition-all"
              style={{ borderColor: '#e2e8f0' }}
            >
              <svg className="w-5 h-5 text-slate-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <div>
                ...
                <p className="text-sm font-medium text-slate-700 group-hover:text-slate-900 transition-colors">Guitar Track Home</p>
              </div>
            </Link>
          )}
          ...
          {nextSection ? (
            <Link
              href={`/curriculum/semester-1/guitar/${nextSection.slug}`}
            ...
            >
              <div>
                ...
                <p className="text-sm font-medium text-slate-700 group-hover:text-slate-900 transition-colors">Back to Guitar Track Home</p>
              </div>
              <svg className="w-5 h-5 text-slate-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
              </svg>
