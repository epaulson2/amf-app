...
const FILE = '04-curriculum/semester-1/core-curriculum.md'

const SECTIONS = [
  { slug: 'overview', title: 'Overview & Framework', prev: null, next: 'month-1' },
  { slug: 'month-1', title: 'Month 1: Stabilize', prev: 'overview', next: 'month-2' },
  { slug: 'month-2', title: 'Month 2: Vary', prev: 'month-1', next: 'month-3' },
  { slug: 'month-3', title: 'Month 3: Adapt', prev: 'month-2', next: 'reference' },
  { slug: 'reference', title: 'Materials & Reference', prev: 'month-3', next: null },
]
...
async function getSectionContent(slug: string): Promise<string> {
  switch (slug) {
    case 'overview':
      return getMarkdownSection(FILE, null, ['Month Arc: Stabilize'])
    case 'month-1':
      return getMarkdownSections(FILE, [
        { from: 'Month 1: Stabilize', stop: ['Month 2: Vary'] },
        { from: 'Month 1: Stabilize (Weeks', stop: ['Month 2: Vary (Weeks'] },
      ])
    case 'month-2':
      return getMarkdownSections(FILE, [
        { from: 'Month 2: Vary', stop: ['Month 3: Adapt'] },
        { from: 'Month 2: Vary (Weeks', stop: ['Month 3: Adapt (Weeks'] },
      ])
    case 'month-3':
      return getMarkdownSections(FILE, [
        { from: 'Month 3: Adapt', stop: ['Primary and Secondary'] },
        { from: 'Month 3: Adapt (Weeks', stop: ['Daily Practice Rhythm'] },
      ])
    case 'reference':
      return getMarkdownSections(FILE, [
        { from: 'Primary and Secondary Instrument Hierarchy', stop: ['Week-by-Week Focus'] },
        { from: 'Daily Practice Rhythm', stop: [] },
      ])
    default:
      return ''
  }
...
export default async function CoreSectionPage({
  params,
}: {
    ...
    <div style={{ backgroundColor: '#f8f7f4', minHeight: '100vh' }}>
      ...
      <div className="py-10 px-4 sm:px-6" style={{ backgroundColor: '#0f172a' }}>
        <div className="max-w-3xl mx-auto">
          <p className="text-slate-400 text-sm mb-3">
            ...
            {' / '}
            <Link href="/curriculum/semester-1/core" className="hover:text-amber-400 transition-colors">Core Curriculum</Link>
            {' / '}
      ...
      <div className="border-t" style={{ borderColor: '#e2e8f0', backgroundColor: '#fff' }}>
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 flex items-center justify-between gap-4">
          {prevSection ? (
            <Link
              href={`/curriculum/semester-1/core/${prevSection.slug}`}
            ...
            <Link
              href="/curriculum/semester-1/core"
              className="flex-1 group flex items-center gap-3 p-4 rounded-xl border hover:border-slate-300 transition-all"
              style={{ borderColor: '#e2e8f0' }}
            >
              <svg className="w-5 h-5 text-slate-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <div>
                ...
                <p className="text-sm font-medium text-slate-700 group-hover:text-slate-900 transition-colors">Core Curriculum Home</p>
              </div>
            </Link>
          )}
          ...
          {nextSection ? (
            <Link
              href={`/curriculum/semester-1/core/${nextSection.slug}`}
            ...
            >
              <div>
                ...
                <p className="text-sm font-medium text-slate-700 group-hover:text-slate-900 transition-colors">Back to Core Curriculum Home</p>
              </div>
              <svg className="w-5 h-5 text-slate-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
              </svg>
